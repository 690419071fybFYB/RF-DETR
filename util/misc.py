...
class MetricLogger(object):
    ...
    def format_meters(self, indent: int = 4) -> str:
        pad = " " * indent
        if not self.meters:
            return f"{pad}<no metrics>"
        lines = [f"{pad}{name}: {meter}" for name, meter in sorted(self.meters.items())]
        return "\n".join(lines)
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        base_log = (
            ...
            "  time: {time}  data: {data}{memory_line}\n"
            "{meters}"
        )
        MB = 1024.0 * 1024.0
        for obj in iterable:
            ...
            if i % print_freq == 0 or i == len(iterable) - 1:
                ...
                if torch.cuda.is_available():
                    memory_line = f"  max mem: {torch.cuda.max_memory_allocated() / MB:.0f}MB"
                else:
                    memory_line = ""
                log_msg = base_log.format(
                    i,
                    len(iterable),
                    eta=eta_string,
                    meters=self.format_meters(),
                    time=str(iter_time),
                    data=str(data_time),
                    header=header,
