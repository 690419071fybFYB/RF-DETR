...
def format_args(args: argparse.Namespace) -> str:
    """
    Formats argparse.Namespace for readable logging.
    Falls back to default repr if unexpected values break pformat.
    """
    try:
        return pformat(vars(args), indent=2, sort_dicts=False, width=100)
    except Exception:
        return repr(args)
