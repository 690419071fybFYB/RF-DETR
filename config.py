...
class RFDETR800Config(RFDETRBaseConfig):
    """
    RF-DETR configuration tuned for square 800 inputs.
    Uses larger patches/windows so the encoder sees images
    that are cleanly divisible by the ViT tiling scheme.
    """
    resolution: int = 800
    patch_size: int = 16
    num_windows: int = 5
    positional_encoding_size: int = 50
    pretrain_weights: Optional[str] = None
