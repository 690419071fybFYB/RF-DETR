...
def get_next_run(folder_name):
    """
    Determine the next run index that needs to be executed.
    A run is considered complete if run_{i}/final_info.json exists.
    """
    for run_idx in range(1, MAX_RUNS + 1):
        run_dir = osp.join(folder_name, f"run_{run_idx}")
        if not osp.exists(run_dir):
            return run_idx
        if not osp.exists(osp.join(run_dir, "final_info.json")):
            return run_idx
    return MAX_RUNS + 1
...
def perform_experiments(idea, folder_name, coder, baseline_results) -> bool:
    ## RUN EXPERIMENT
    current_iter = 0
    # If resuming, start_run may be > 1
    start_run = get_next_run(folder_name)
    if start_run > MAX_RUNS:
        print("All runs already completed. Skipping experiments.")
        return True
    run = start_run
